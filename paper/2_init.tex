...
\subsection{The Absolute Scale: Unit Conversion}
The geometric ratio $H_{sys}/\Delta$ is dimensionless. To convert this to the physical units of an expansion rate (inverse time), it must be scaled by a fundamental mass squared, divided by a fundamental mass (in natural units where $\hbar=c=1$). The framework provides only two such fundamental mass scales derived in Paper I: the electron mass ($m_e$, the persistence margin) and the Planck mass ($M_P$, the saturation limit). The Hubble constant is thus fixed by this unique combination:
\begin{equation}
H_0 = \kappa \cdot \frac{m_e^2}{M_P}
\end{equation}
where $\kappa$ is a geometric factor of order unity. The early-universe rate corresponds to the case where $\kappa = H_{sys}/\Delta$. This is not a fit; it is a direct conversion from the geometric rate to physical units using the fundamental scales of the theory.
...
\begin{enumerate}
    ...
    \item \textbf{Geometric Dilution ($\zeta$):} This bias is diluted across the total number of available geometric ``slots" in the initial inflationary phase space. This is not a spatial volume, but a dimensionless count of available states, given by the product of the resonant circumference ($\pi\Delta$) and the effective structural channel count ($H_{sys} + \chi/D$).
    \begin{equation}
    \begin{split}
...
These stresses are geometrically orthogonal—one acts on the discrete channel count, the other on the continuous manifold curvature—and therefore sum linearly. The total systemic overhead is the linear sum of the independent costs required to overcome the digital and analog projection barriers.
\begin{equation}
