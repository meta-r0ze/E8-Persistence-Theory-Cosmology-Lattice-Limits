...
Dark Matter is not a particle but the total mass-energy of the $E_8$ substrate itself, which gravitationally dominates the energy of the ``software" (the Standard Model fermions) it supports. The abundance ratio is predicted by comparing the total degrees of freedom of the hardware to those of the software. The $E_8$ lattice has a total capacity represented by its 248 dimensions. The Standard Model, in its final stable state, requires three generations, each occupying a $\nu=16$ chiral state space, for a total of $3 \times 16 = 48$ fundamental fermionic states. The ratio is therefore:
\begin{equation}
    \frac{\Omega_{DM}}{\Omega_b} = \frac{\text{Hardware Capacity}}{\text{Software States}} = \frac{248}{48} \approx 5.167
\end{equation}
